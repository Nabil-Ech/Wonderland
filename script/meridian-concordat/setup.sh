...
CAPSULE_TX=$(cast send $CANNON_GUARD \
  "createCapsule(address,address,bytes4,uint256)" \
  $HELIX $MRC "0x40c10f19" 0 \
  --private-key $SYSTEM_KEY --rpc-url $RPC --json 2>/dev/null)
CAPSULE_BLOCK=$(echo "$CAPSULE_TX" | python3 -c "import sys,json; print(json.load(sys.stdin)['blockNumber'])" 2>/dev/null || echo "latest")
# Read capsule address from CapsuleCreated event topic[2] (the capsule address)
CAPSULE_LOG=$(cast logs --from-block $CAPSULE_BLOCK --to-block $CAPSULE_BLOCK \
  --address $CANNON_GUARD --rpc-url $RPC 2>/dev/null)
# topic[2] is the capsule address — extract 2nd topic after the event sig
CAPSULE="0x$(echo "$CAPSULE_LOG" | grep -A2 "topics:" | tail -1 | grep -oP '[0-9a-fA-F]{64}' | head -1 | tail -c 41)"
if [[ ! "$CAPSULE" =~ ^0x[0-9a-fA-F]{40}$ ]]; then
  # Fallback: read capsule count and get the first capsule for HELIX
  CAPSULE=$(cast call $CANNON_GUARD "getCapsules(address)(address[])" $HELIX --rpc-url $RPC | tr -d '[]' | xargs | cut -d' ' -f1)
fi
